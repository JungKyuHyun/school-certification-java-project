	...
	public void selectMenu() {
		do {
			...
			if(certificate.paperStatus() <= (int)(certificate.getPaperDefault()*0.2 ) && certificate.paperStatus() !=0) {
				...
				System.out.println("*** 이제부터 장바구니 기능 및 결제를 사용할 수 없습니다. ***");
				System.out.println("*** 관리자에게 문의 하세요 1588-3082 ***\n");
				sum=0;i=0;
			}else if(certificate.paperStatus()==0) {
				System.out.println("*** 용지 부족으로 [종이 출력]이 불가능 합니다. ***");
				System.out.println("*** 이제부터 장바구니 기능 및 결제를 사용할 수 없습니다. ***");
				System.out.println("*** 관리자에게 문의 하세요 1588-3082 ***\n");
...
//				Login.loginDisplay();
			...
			if(i != 0 && certificate.paperStatus() > (int)(certificate.getPaperDefault()*0.2 )) {System.out.println("0. 장바구니 결제하기["+sum+"원]");}
...
//			case "7": System.out.println("------------------------------\n"); power(false);break;
			case "0": if(sum!=0 && certificate.paperStatus() > (int)(certificate.getPaperDefault()*0.2 )) {cardPayment(sum); selectMenuThird(menu);break;}
...
//			this.i++;
	...
	private void emailTrans(String MenuNumber) {
			...
			if(email.validate(emailAddress) == true && shoppingBaskel[0]!=null && temp !=1 && certificate.paperStatus()!=0) {
				System.out.println("\n------------------------------");
				System.out.print("\n선택하신 " + Arrays.toString(Arrays.copyOf(shoppingBaskel, i)) + "가 ");
				System.out.println("\n이메일("+emailAddress +")로 전송되었습니다.\n");
				System.out.println("------------------------------");
				sum=0; i=0;
			}else if(email.validate(emailAddress) == true && temp==1 | certificate.paperStatus()==0) {//shoppingBaskel[0]==null ){
				System.out.println("\n------------------------------");
...
//				sum=0; i=0;
	...
	private void selectMenuSecond(String menuNumberFirst) {
		...
		if(certificate.paperStatus() > (int)(certificate.getPaperDefault()*0.2 )) {
			System.out.println("2. 장바구니에 담기("+this.i+"/"+this.shoppingBaskel.length+")\n");
		}
		if(i!=0 && certificate.paperStatus() > (int)(certificate.getPaperDefault()*0.2 )) {
		...
		do {
			...
			case "2" : if(certificate.paperStatus() > (int)(certificate.getPaperDefault()*0.2 )) {
				ShoppingBasket(menuNumberFirst); break;
			}
			case "3" : if(i!=0 && certificate.paperStatus() > (int)(certificate.getPaperDefault()*0.2 )) {
	...
	private void selectMenuThird(String menuNumberFirst) {
		String tempD = menuNumberFirst;
		System.out.println("------------------------------");
		System.out.println();
		System.out.println("    [출력 메뉴]\n");
		System.out.println("1. 이메일 전송");
		if(certificate.paperStatus()!=0) {
